...
# Script para generar gráfico de la actividad de la lista Ubuntu Users


library(readr)
library(ggplot2)

ubuntucount <- read_csv("../datos/ubuntuuserscount.csv", 
                          col_types = cols(Fecha = col_date(format = "%Y-%B")))

ubuntuusers <- ubuntucount[order(as.Date(ubuntucount$Fecha, format="%Y-%B")),] 
attach(ubuntuusers)
...
ggplot( data = ubuntuusers, aes( Fecha, Cantidad ))  + geom_bar(stat = "identity", fill="red4", alpha=1) + ggtitle("Mensajes mensuales enviados a la lista de discusión de Ubuntu") + scale_y_continuous(breaks=seq(0,7000,500)) + theme(axis.ticks = element_line(size = .2), axis.text.x  = element_text(size=6,angle=90,colour="black",vjust=1,hjust=1), plot.title = element_text(face="bold",hjust = 0.5)) + scale_x_date(breaks=parada.vec, date_labels = format(parada.vec,"%b %Y")) + labs(x="Mes - Año", y="Número de mensajes")
